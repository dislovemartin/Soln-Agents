const ContextMenuModel = {
  async create(workspaces) {
    ...
    if (workspaces && workspaces.length > 0) {
      ...
      chrome.contextMenus.create({
        id: "saveEntirePageToSolnAI",
        title: "Save entire page to SolnAI",
        contexts: ["page"],
      });
...
const ExtensionModel = {
  ...
  async saveToSolnAI(selectedText, pageTitle, pageUrl) {
    const { apiBase, apiKey } = await chrome.storage.sync.get([
      "apiBase",
      "apiKey",
    ]);
  ...
  async saveEntirePageToSolnAI(pageContent, pageTitle, pageUrl) {
    const { apiBase, apiKey } = await chrome.storage.sync.get([
      "apiBase",
      "apiKey",
    ]);
  ...
  async handleResponse(response, action) {
    if (response.status === 401 || response.status === 403) {
      ...
      this.showNotification(
        "success",
        "Successfully saved content to SolnAI."
      );
    }
  },
  ...
  showNotification(type, message) {
    ...
    setTimeout(() => {
      ...
      chrome.action.setTitle({ title: "SolnAI Extension" });
    }, 5000);
  },
};
...
chrome.contextMenus.onClicked.addListener((info, tab) => {
  if (info.menuItemId === "saveToSolnAI") {
    ExtensionModel.saveToSolnAI(info.selectionText, tab.title, tab.url);
    return;
  }
  ...
  if (info.menuItemId === "saveEntirePageToSolnAI") {
    getPageContent(tab.id)
      .then((content) => {
        ExtensionModel.saveEntirePageToSolnAI(content, tab.title, tab.url);
      })
      .catch((error) => {
