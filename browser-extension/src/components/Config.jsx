...
export default function Config({ status, onStatusChange }) {
  ...
  async function disconnectFromExtension() {
    ...
    onStatusChange();
    setSaveStatus("Successfully disconnected from SolnAI");
  ...
  const handleConnect = async () => {
    try {
      ...
      if (!online) {
        setSaveStatus(
          "SolnAI is currently offline. Please try again later."
        );
        return;
      }
      ...
      onStatusChange();
      setSaveStatus("Successfully connected to SolnAI");
      chrome.runtime.sendMessage({ action: "connectionUpdated" });
    } catch (error) {
