...
2. After successful login, Run the following command to create a deployment using the Deployment Manager CLI:
  ...
  gcloud deployment-manager deployments create solnai-deployment --config gcp/deployment/gcp_deploy_solnai.yaml
...
```
gcloud compute instances get-serial-port-output solnai-instance 
```

ssh into the instance

```
gcloud compute ssh solnai-instance 
```

Delete the deployment
```
gcloud deployment-manager deployments delete solnai-deployment 
```
