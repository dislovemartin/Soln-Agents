...
_The following risks are relevant to any AI using your computer, not just SolnAI_
...
The <b>Computer use</b> feature for SolnAI is an experimental feature that allows you to enable an AI to use your computer to complete tasks.
...
- **Accessibility:** (MacOS only) This feature requires the `Accessibility` and `Screen Recording` permissions to be enabled for SolnAI.
...
You can do this by clicking the pause icon in the UI, pressing `CMD+K` or `CTRL+K`, or by quitting the SolnAI application.
...
_This section is relevant to users running SolnAI Desktop on MacOS_
...
In order to use the computer use feature, you need to have the `Accessibility` permissions enabled for SolnAI on your system.

This is done by opening the `Security & Privacy` settings on MacOS and clicking on the `Privacy` tab. From there, find `Accessibility` on the left and click on the `+` button to add SolnAI.

This will allow SolnAI to control your computer's mouse and keyboard.
...
In order to use the computer use feature, you need to have the `Screen Recording` permissions enabled for SolnAI on your system.

This is done by opening the `Security & Privacy` settings on MacOS and clicking on the `Privacy` tab. From there, find `Screen Recording` on the left and click on the `+` button to add SolnAI.

This will allow SolnAI to take screenshots of your display.
...
Once you have enabled the feature and configured it with your API key, you can invoke computer use by typing in `@os` in the SolnAI chat along with a prompt.
...
Once the OS agent is running, SolnAI will minimize to get out of the way and you should see a popup in your display allowing you to control or halt the OS agent.
...
You can also quit the SolnAI application which will halt the OS agent as well. You can drag the popup around to get it out of the way, but this may interfere with the OS agent's ability to control your mouse position if needed.
...
The OS agent will output its actions and any relevant information to the SolnAI chat as it executes. These actions are currently **not** saved or stored your workspace's chat history.
