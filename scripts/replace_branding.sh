#!/bin/bash
# Replace SolnAI with SolnAI across various file types
...
for TYPE in "${FILE_TYPES[@]}"; do
  ...
  find . -name "$TYPE" -type f -not -path "*/node_modules/*" -not -path "*/.git/*" | while read -r FILE; do
    # Check if file contains SolnAI (case-insensitive)
    if grep -q -i "SolnAI" "$FILE" || grep -q "anything-llm" "$FILE"; then
      # Count replacements for reporting
      REPLACED_COUNT=0

      # Perform replacements
      sed -i 's/SolnAI/SolnAI/g' "$FILE"
